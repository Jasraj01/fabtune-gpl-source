...
fun parseChallengeData(rawChallengeData: String): String {
    ...
    val safeScriptWrappedValue = challengeData[1]
        .takeIf { it !is JsonNull }
        ?.jsonArray
        ?.find { it.jsonPrimitive.isString }
    val trustedResourceUrlWrappedValue = challengeData[2]
        .takeIf { it !is JsonNull }
        ?.jsonArray
        ?.find { it.jsonPrimitive.isString }
        ...
        JsonObject(
            mapOf(
                ...
                "interpreterJavascript" to JsonObject(
                    mapOf(
                        "privateDoNotAccessOrElseSafeScriptWrappedValue" to (safeScriptWrappedValue ?: JsonNull),
                        "privateDoNotAccessOrElseTrustedResourceUrlWrappedValue" to (trustedResourceUrlWrappedValue ?: JsonNull),
                    ),
                ),
