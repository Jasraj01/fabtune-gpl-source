...
fun OnlineSearchResult(
    ...
    val onBackPressed = remember(navController) {
        {
            navController.popBackStack(route = "explore", inclusive = false)
            Unit
        }
    }
    BackHandler(onBack = onBackPressed)

    val onSearch: (String) -> Unit = remember(navController, focusManager, decodedQuery) {
        { searchQuery ->
    ...
    val leadingIcon: @Composable () -> Unit = remember(onBackPressed) {
        {
    ...
    val trailingIcon: @Composable () -> Unit = remember(query.text) {
        {
    ...
    val itemsPage by remember(searchFilter, viewModel.viewStateMap) {
        androidx.compose.runtime.derivedStateOf {
            searchFilter?.value?.let { viewModel.viewStateMap[it] }
        }
    }
    val filteredItems = remember(itemsPage) {
        itemsPage?.items.orEmpty().distinctBy { it.id }
    }

    LaunchedEffect(lazyListState, searchFilter) {
        snapshotFlow {
            lazyListState.layoutInfo.visibleItemsInfo.any { it.key == "loading" }
        }
            .distinctUntilChanged()
            .collect { shouldLoadMore ->
                if (!shouldLoadMore || searchFilter == null) return@collect
                viewModel.loadMore()
            }
    }
                ...
                LazyColumn(
                    ...
                    if (searchFilter == null) {
                        searchSummary?.summaries?.forEachIndexed { summaryIndex, summary ->
                            item(
                                key = "summary_title_${summaryIndex}_${summary.title}",
                                contentType = "summary_title",
                            ) {
                            ...
                            itemsIndexed(
                                ...
                                itemContent = { _, item -> ytItemContent(item) },
                            )
                        }

                        if (searchSummary?.summaries?.isEmpty() == true) {
                            item(key = "empty_summary", contentType = "empty") {
                                EmptyPlaceholder(
